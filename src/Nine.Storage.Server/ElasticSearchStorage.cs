namespace Nine.Storage
{
    ...
    public class ElasticSearchStorage<T> : IStorage<T> where T : class
    {
        ...
        private static readonly string _typeName = typeof(T).Name.ToLowerInvariant();
        ...
        public ElasticSearchStorage(string endpoint, string indexName = "default")
        {
            ...
            _client = new ElasticClient(new ConnectionSettings(new Uri(endpoint), indexName.ToLowerInvariant()));
        }

        public async Task<T> Get(string key)
        {
            var response = await _client.GetAsync<T>(key, null, _typeName).ConfigureAwait(false);
        ...
        {
            var response = await _client.IndexAsync(value, t => t.Id(key).OpType(OpType.Create)).ConfigureAwait(false);
        ...
        {
            var response = await _client.IndexAsync(value, t => t.Id(key)).ConfigureAwait(false);
            EnsureSuccess(response);
        }

        public async Task<bool> Delete(string key)
        {
            var response = await _client.DeleteAsync(null, _typeName, key).ConfigureAwait(false);
            EnsureSuccess(response);
